...
class AppController {
  constructor() {
    ...
    this._sendPushOptions = "";
    // document.querySelector('.js-send-push-options');
    // this._subscriptionJSONCode = document.querySelector(
    //   '.js-subscription-json');
    this._payloadTextField = "";
    // document.querySelector('.js-payload-textfield');
  ...
  _uiInitialised(toggleSwitch) {
    ...
    document
      .querySelector(".js-push-toggle-switch > input")
  ...
  _subscriptionUpdate(subscription) {
    ...
    this._subscriptionJSONCode.textContent = JSON.stringify(
      subscription,
      null,
      2
    );
  ...
  updatePushInfo() {
    ...
    const payloadText = this._payloadTextField;
    return this._encryptionHelper
      .getRequestDetails(this._currentSubscription, payloadText)
      .then(requestDetails => {
        ...
        Object.keys(requestDetails.headers).forEach(header => {
          ...
          liElement.innerHTML =
            `<span>${header}</span>: ` + `${requestDetails.headers[header]}`;
        ...
        if (requestDetails.body && requestDetails.body instanceof ArrayBuffer) {
          ...
          curlCommand = null;
          curlError =
            "Sorry, but because the web push " +
  ...
  showErrorMessage(title, message) {
    ...
    //   const pushOptionsContainer = document.querySelector(
    //     ".js-send-push-options"
    //   );
