<div class="card cardsection">
    <h5 class="card-header main-color-bg">Tổng quan</h5>
    <div class="card-body">
        
        <div class="row">
            
        <div class="col-md-3 ">
            <div class="border main-card-overview">
                <h2><i class="fa fa-newspaper-o"></i><?php echo DemBan($MaKH);?></h2>
                <h4>Số bài đăng bán</h4>
            </div>
        </div>

        <div class="col-md-3">
            <div class="border  main-card-overview">
                <h2><i class="fa fa-newspaper-o"></i><?php echo DemMua($MaKH);?></h2>
                <h4>Số bài đăng mua</h4>
            </div>
        </div>

        <div class="col-md-3">
                <div class="border  main-card-overview">
                    <h2><i class="fa fa-users"></i><?php echo DemTheoDoi($MaKH);?></h2>
                    <h4>Người theo dõi</h4>
                </div>
        </div>

        <div class="col-md-3">
                    <div class="border  main-card-overview">
                        <h2><i class="fa fa-users"></i><?php echo DemDangTheoDoi($MaKH);?></h2>
                        <h4>Đang theo dõi</h4>
                    </div>
        </div>
    </div>
    </div>
    </div>


    <!--Thông báo-->
    <div class="card cardsection" style="margin-top: 15px;">
        <h5 class="card-header main-color-bg">Thông báo từ người theo dõi</h5>
        <div class="card-body">
            <table class="table table-striped">
                <thead>
                    <tr>
                    <th scope="col">Tiêu đề</th>
                    <th scope="col">Hình ảnh</th>  
                    <th scope="col">Thời gian</th>
                    <th scope="col">Người đăng</th>
                    <th scope="col">Xem</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                    <td>Bán mũ của người yêu</td>
                    <td></td>
                    <td>12/1/2018</td>
                    <td>Allan Nguyễn</td>
                    <td><a class="btn btn-primary" href="#" style="color:white;">Xem</a></td>
                    </tr>       
                </tbody>
                </table>
        </div>
    </div>

